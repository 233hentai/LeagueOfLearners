...
private:
	...
	void ClearTree();
	UUserWidget* CreateWidgetForNode(const ITreeNodeInterface* Node, class UCanvasPanelSlot*& OutSlot);
	void CreateConnection(const UUserWidget* From, const UUserWidget* To);
	void DrawStream(bool bUpperStream, const ITreeNodeInterface* StartingNodeInterface, UUserWidget* StartingNodeWidget,
		UCanvasPanelSlot* StartingNodeSlot, int StartingNodeDepth,float& NextLeafPositionX, TArray<UCanvasPanelSlot*>& OutStreamSlot);
};
