...
void ATargetActor_Line::StartTargeting(UGameplayAbility* Ability)
{
	...
	if (HasAuthority()) {
		GetWorldTimerManager().SetTimer(TargetingTimerHandle, this, &ATargetActor_Line::DoTargetCheckAndReport, TargetingInterval, true);
	}
}
...
{
	if (GetWorld() && TargetingTimerHandle.IsValid()) {
...
void ATargetActor_Line::UpdateTargetTrace()
{
	...
	FCollisionResponseParams CollisionResponse(ECR_Overlap);
	GetWorld()->SweepMultiByChannel(HitResults, GetActorLocation(), SweepEndLocation, FQuat::Identity, ECC_WorldDynamic, FCollisionShape::MakeSphere(DetectionCylinderRadius), QueryParams, CollisionResponse);
	...
	if (LazerVFX) {
		LazerVFX->SetVariableFloat(LazerVFXParamName, LineLength/100.f);
	}
}
